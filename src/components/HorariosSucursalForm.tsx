  ...
  if (isLoading) {
    return (
      <div className="fixed inset-0 bg-black bg-opacity-50 flex items-center justify-center p-4 z-50">
        <div className="bg-qoder-dark-bg-secondary rounded-none p-6 w-full max-w-md">
    ...
    <div className="fixed inset-0 bg-black bg-opacity-50 flex items-center justify-center p-4 z-50">
      <div className="bg-qoder-dark-bg-secondary rounded-2xl border border-qoder-dark-border-primary p-6 w-full max-w-4xl max-h-[90vh] overflow-y-auto shadow-2xl">
        <div className="flex justify-between items-center mb-6 pb-4 border-b border-qoder-dark-border-primary">
          ...
          <button
            onClick={onClose}
            className="text-qoder-dark-text-secondary hover:text-qoder-dark-text-primary text-2xl p-2 rounded-none hover:bg-qoder-dark-bg-primary transition-colors duration-200"
          >
            &times;
          </button>
        </div>
        ...
        {error && (
          <div className="mb-6 p-4 bg-red-500/20 border border-red-500/50 text-red-200 rounded-none">
        ...
        <form onSubmit={handleSubmit}>
          <div className="space-y-5">
            {diasSemana.map(dia => (
              <div
                key={dia.id}
                className={`bg-gradient-to-br from-qoder-dark-bg-form to-qoder-dark-bg-secondary rounded-2xl border transition-all duration-300 hover:shadow-lg overflow-hidden ${horariosForm[dia.id]?.activo
                  ? "border-qoder-dark-border-primary shadow-sm"
                  : "border-qoder-dark-border-secondary opacity-70"
                  }`}
              >
                <div
                  className="flex items-center justify-between p-5 cursor-pointer hover:bg-qoder-dark-bg-primary/30 transition-colors"
                  onClick={() => toggleDayExpansion(dia.id)}
                >
                  <div className="flex items-center">
                    <div className={`p-1.5 rounded-none mr-3 transition-colors ${expandedDays[dia.id] ? 'bg-qoder-dark-accent-primary/10 text-qoder-dark-accent-primary' : 'text-qoder-dark-text-secondary'}`}>
                      {expandedDays[dia.id] ? (
                    ...
                    <label className="flex items-center cursor-pointer group">
                      <div className="relative">
                        <input
                          ...
                          className="sr-only"
                        />
                        <div className={`block w-12 h-6 rounded-none transition-colors duration-300 ${horariosForm[dia.id]?.activo
                          ? "bg-qoder-dark-accent-primary"
                          : "bg-qoder-dark-bg-primary"
                          }`}></div>
                        <div className={`absolute left-1 top-1 bg-white w-4 h-4 rounded-none transition-transform duration-300 ${horariosForm[dia.id]?.activo ? "transform translate-x-6" : ""
                          }`}></div>
                      </div>
                      <span className={`ml-2 text-sm font-medium ${horariosForm[dia.id]?.activo
                ...
                <AnimatePresence>
                  {expandedDays[dia.id] && (
                    ...
                    >
                      <div className="px-5 pb-5 pt-0 border-t border-qoder-dark-border-primary/30">
                        <div className="grid grid-cols-1 md:grid-cols-2 gap-4 mt-5">
                          <div className="space-y-2">
                            ...
                            <input
                              ...
                              className="w-full bg-qoder-dark-bg-primary border border-qoder-dark-border-primary rounded-none px-4 py-3 text-qoder-dark-text-primary focus:ring-2 focus:ring-qoder-dark-accent-primary focus:border-transparent transition-all duration-200"
                            ...
                            <div className="flex items-center justify-between mb-3">
                              <div className="flex items-center">
                                ...
                                <Checkbox
                                  ...
                                  disabled={!horariosForm[dia.id]?.activo}
                                  label="Configurar hora de almuerzo"
                                />
                              </div>
                            </div>

                            {tieneAlmuerzo[dia.id] && horariosForm[dia.id]?.activo && (
                              <div className="grid grid-cols-1 md:grid-cols-2 gap-4 mt-3 p-4 bg-qoder-dark-bg-primary/50 rounded-none border border-qoder-dark-border-secondary">
                                <div className="space-y-2">
                                  <div className="flex items-center">
                                  ...
                                  <input
                                    ...
                                    className="w-full bg-qoder-dark-bg-secondary border border-qoder-dark-border-primary rounded-none px-4 py-3 text-qoder-dark-text-primary focus:ring-2 focus:ring-yellow-500 focus:border-transparent transition-all duration-200"
                                  />
                                </div>
          ...
          <div className="flex flex-col sm:flex-row justify-end space-y-3 sm:space-y-0 sm:space-x-4 mt-8 pt-6 border-t border-qoder-dark-border-primary">
            <button
              type="button"
              onClick={onClose}
              className="px-5 py-3 bg-qoder-dark-bg-primary text-qoder-dark-text-primary rounded-none hover:bg-qoder-dark-bg-hover border border-qoder-dark-border-primary transition-all duration-200 hover:shadow-md flex items-center justify-center"
              disabled={guardando}
            >
            ...
            <button
              type="submit"
              className="px-5 py-3 bg-qoder-dark-accent-primary text-white rounded-none hover:bg-qoder-dark-accent-hover disabled:opacity-50 transition-all duration-200 hover:shadow-lg flex items-center justify-center"
              disabled={guardando}
            >
              {guardando ? (
                <>
